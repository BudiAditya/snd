    ...
    public function recap() {
        ...
        if (count($this->getData) > 0) {
            ...
            $this->connector->CommandText =
                ...
                WHERE a.company_id = ".$this->userCompanyId." Order By a.kode;";
            ...
            $this->connector->CommandText =
                ...
                SELECT a.acc_code, SUM(a.db_amount) AS 'total_debit', SUM(IF(a.wbs_post = 'MTC', a.db_amount, 0)) AS 'mtc_debit', SUM(IF(a.wbs_post = 'ACR', a.db_amount, 0)) AS 'acr_debit', SUM(IF(a.wbs_post = 'ADJ', a.db_amount, 0)) AS 'adj_debit'
                FROM vw_ac_journal_detail_4report AS a
                WHERE a.journal_status = 2 AND a.is_deleted = 0 AND a.journal_date BETWEEN ?start AND ?end
                    AND a.acc_code IN (SELECT acc_no FROM acc_id)
                GROUP BY a.acc_code;";
            $this->connector->ExecuteNonQuery();
            ...
            $this->connector->CommandText =
                ...
                SELECT a.acc_code, SUM(a.cr_amount) AS 'total_credit', SUM(IF(a.wbs_post = 'MTC', a.cr_amount, 0)) AS 'mtc_credit', SUM(IF(a.wbs_post = 'ACR', a.cr_amount, 0)) AS 'acr_credit', SUM(IF(a.wbs_post = 'ADJ', a.cr_amount, 0)) AS 'adj_credit'
                FROM vw_ac_journal_detail_4report AS a
                WHERE a.journal_status = 2 AND a.is_deleted = 0 AND a.journal_date BETWEEN ?start AND ?end
                    AND a.acc_code IN (SELECT acc_no FROM acc_id)
                GROUP BY a.acc_code;";
            ...
            if ($rekapMethod == 1 && $month > 1) {
                ...
                $this->connector->CommandText =
                    ...
                    SELECT a.acc_code, SUM(a.db_amount) AS total_debit_prev
                    FROM vw_ac_journal_detail_4report AS a
                    WHERE a.journal_status = 2 AND a.is_deleted = 0 AND a.journal_date BETWEEN ?firstJan AND ?prev AND a.acc_code IN (SELECT acc_no FROM acc_id)
                    GROUP BY a.acc_code;";
                $this->connector->ExecuteNonQuery();
                ...
                $this->connector->CommandText =
                    ...
                    SELECT a.acc_code, SUM(a.cr_amount) AS total_credit_prev
                    FROM vw_ac_journal_detail_4report AS a
                    WHERE a.journal_status = 2 AND a.is_deleted = 0 AND a.journal_date BETWEEN ?firstJan AND ?prev AND a.acc_code IN (SELECT acc_no FROM acc_id)
                    GROUP BY a.acc_code;";
                $this->connector->ExecuteNonQuery();
                ...
                // #06: OK final query...
                $this->connector->CommandText =
                    "SELECT a.*, b.total_debit, b.mtc_debit,c.mtc_credit,b.acr_debit,c.acr_credit,b.adj_debit,c.adj_credit, d.total_debit_prev, e.total_credit_prev, f.db_amount as bal_debit_amt, f.cr_amount as bal_credit_amt
                    FROM acc_id AS a
                        ...
                        LEFT JOIN vw_ac_saldoawal AS f ON a.acc_no = f.acc_no AND f.op_date = ?firstJan And f.company_id = ".$this->userCompanyId."
                    ORDER BY a.acc_no;";
            } else {
                ...
                $this->connector->CommandText =
                    "SELECT a.*, b.total_debit, b.mtc_debit,c.mtc_credit,b.acr_debit,c.acr_credit,b.adj_debit,c.adj_credit, 0 AS total_debit_prev, 0 AS total_credit_prev, f.db_amount as bal_debit_amt, f.cr_amount as bal_credit_amt
                    FROM acc_id AS a
                        LEFT JOIN sum_debit AS b ON a.acc_no = b.acc_code
                        LEFT JOIN sum_credit AS c ON a.acc_no = c.acc_code
                        LEFT JOIN vw_ac_saldoawal AS f ON a.acc_no = f.acc_no AND f.op_date = ?firstJan And f.company_id = ".$this->userCompanyId."
