...
class SoController extends AppController {
    ...
    public function index() {
        ...
        $settings["columns"][] = array("name" => "if(a.so_status = 0,'Draft',if(a.so_status = 1,'Open','Closed'))", "display" => "Status", "width" => 40);
        ...
        $settings["filters"][] = array("name" => "if(a.so_status = 0,'Draft','Posted')", "display" => "Status");
        ...
        if (!$router->IsAjaxRequest) {
            ...
            if ($acl->CheckUserAccess("ar.so", "add")) {
                $settings["actions"][] = array("Text" => "Add", "Url" => "ar.so/add", "Class" => "bt_add", "ReqId" => 0);
            }
            if ($acl->CheckUserAccess("ar.so", "edit")) {
                $settings["actions"][] = array("Text" => "Edit", "Url" => "ar.so/edit/%s", "Class" => "bt_edit", "ReqId" => 1,
                    "Error" => "Maaf anda harus memilih Data Order terlebih dahulu sebelum proses edit.\nPERHATIAN: Pilih tepat 1 data rekonsil",
                    "Confirm" => "");
            }
            if ($acl->CheckUserAccess("ar.so", "delete")) {
                $settings["actions"][] = array("Text" => "Delete", "Url" => "ar.so/delete/%s", "Class" => "bt_delete", "ReqId" => 1);
            }
            if ($acl->CheckUserAccess("ar.so", "view")) {
                $settings["actions"][] = array("Text" => "View", "Url" => "ar.so/view/%s", "Class" => "bt_view", "ReqId" => 1,
                    "Error" => "Maaf anda harus memilih Data Order terlebih dahulu.\nPERHATIAN: Pilih tepat 1 data rekonsil","Confirm" => "");
            }

            $settings["actions"][] = array("Text" => "separator", "Url" => null);
            if ($acl->CheckUserAccess("ar.so", "print")) {
                $settings["actions"][] = array("Text" => "Print SO", "Url" => "ar.so/order_print","Class" => "bt_pdf", "ReqId" => 2, "Confirm" => "Cetak P/O yang dipilih?");
            }

            $settings["actions"][] = array("Text" => "separator", "Url" => null);
            if ($acl->CheckUserAccess("ar.so", "view")) {
                $settings["actions"][] = array("Text" => "Laporan", "Url" => "ar.so/report", "Class" => "bt_report", "ReqId" => 0);
            }
        } else {
        ...
        if (count($this->postData) > 0) {
            ...
            if ($this->ValidateMaster($order)) {
                ...
                if ($rs != 1) {
                    ...
                    redirect_url("ar.so/edit/".$order->Id);
                }
			}
		}
        //load data cabang
	...
	public function add_detail($orderId = null) {
        $order = new So($orderId);
        $orderdetail = new SoDetail();
    ...
    public function report(){
        ...
        if (count($this->postData) > 0) {
            ...
            $order = new So();
            if ($sJnsLaporan == 1){
                $reports = $order->LoadSo4Reports($this->userCompanyId,$sCabangId,$sCustomersId,$sSoStatus,$sStartDate,$sEndDate);
            }elseif ($sJnsLaporan == 2){
                $reports = $order->LoadSo4ReportsDetail($this->userCompanyId,$sCabangId,$sCustomersId,$sSoStatus,$sStartDate,$sEndDate);
            }else{
                $reports = $order->LoadSo4ReportsRekapItem($this->userCompanyId,$sCabangId,$sCustomersId,$sSoStatus,$sStartDate,$sEndDate);
            }
        }else{
            $sCabangId = 0;
            $sCustomersId = 0;
    ...
    public function getjson_solists($customerId){
        $filter = isset($_POST['q']) ? strval($_POST['q']) : '';
        $order = new So();
        $solists = $order->GetActiveSoList($this->userCabangId,$customerId,$filter);
        echo json_encode($solists);
    }

    //proses cetak purchase order (SO)
