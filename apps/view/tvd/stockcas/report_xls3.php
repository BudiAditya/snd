...
if ($userTypeHarga == 0){
    $sheet->setCellValue("F$row","PO Qty");
    $sheet->setCellValue("G$row","SO Qty");
    $sheet->setCellValue("H$row","PO + Stock - SO");
    ...
    $sheet->setCellValue("H$row","PO Qty");
    $sheet->setCellValue("I$row","SO Qty");
    $sheet->setCellValue("J$row","PO + Stock - SO");
...
if ($reports != null){
    while ($rpt = $reports->FetchAssoc()) {
        ...
        if ($userTypeHarga == 0){
            $sheet->setCellValue("F$row",$rpt["po_qty"]);
            $sheet->setCellValue("G$row",$rpt["so_qty"]);
            ...
            $sheet->setCellValue("H$row",$rpt["po_qty"]);
            $sheet->setCellValue("I$row",$rpt["so_qty"]);
            $sheet->setCellValue("J$row","=E$row+H$row-I$row");
    ...
    if ($userTypeHarga > 0){
        ...
        $sheet->getStyle("E$str:J$row")->applyFromArray($idrFormat);
        $sheet->getStyle("A$row:J$row")->applyFromArray(array_merge($allBorders));
        $row++;
    }else{
