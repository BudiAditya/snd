...
class SaldoAwal extends EntityBase {
	...
	public function LoadAll($orderBy = "a.op_no", $includeDeleted = false) {
		$this->connector->CommandText = "SELECT a.* FROM t_ap_saldoawal AS a ORDER BY $orderBy";
	...
	public function FindById($id) {
		$this->connector->CommandText = "SELECT a.* FROM t_ap_saldoawal AS a WHERE a.id = ?id";
	...
	public function Insert() {
		$this->connector->CommandText = 'INSERT INTO t_ap_saldoawal(company_id,supplier_id,op_date,op_no,op_amount,op_status,createby_id,create_time) VALUES(?company_id,?supplier_id,?op_date,?op_no,?op_amount,?op_status,?createby_id,now())';
	...
	public function Update($id) {
		$this->connector->CommandText = 'UPDATE t_ap_saldoawal SET supplier_id = ?supplier_id, op_date = ?op_date, op_amount = ?op_amount, op_status = ?op_status, updateby_id = ?updateby_id, update_time = now() WHERE id = ?id';
        $this->connector->AddParameter("?supplier_id", $this->SupplierId);
        $this->connector->AddParameter("?op_date", $this->OpDate);
        $this->connector->AddParameter("?op_amount", $this->OpAmount);
        $this->connector->AddParameter("?op_status", $this->OpStatus);
	...
	public function Void($id) {
		$this->connector->CommandText = 'UPDATE t_ap_saldoawal SET is_deleted = 1,updateby_id = ?updateby_id, update_time = now() WHERE id = ?id';
		$this->connector->AddParameter("?id", $id);
        $this->connector->AddParameter("?updateby_id", $this->UpdatebyId);
		return $this->connector->ExecuteNonQuery();
	}
    ...
    public function Delete($id) {
        $this->connector->CommandText = 'Delete From t_ap_saldoawal Where id = ?id';
        $this->connector->AddParameter("?id", $id);
        return $this->connector->ExecuteNonQuery();
    }
